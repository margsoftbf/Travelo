        ...
        <div className='relative mt-1 rounded-md shadow-sm'>
            <div className='flex border-2 rounded-md'>
                ...
                <div className='absolute inset-y-0 right-0 gap-1 flex items-center pr-2'>
                    <Minus className='h-4 w-4 text-gray-400 cursor-pointer' aria-hidden='true' onClick={() => setGuest(guest > 1 ? guest - 1 : 1)} />
                    <Plus className='h-4 w-4 text-gray-400 cursor-pointer' aria-hidden='true' onClick={() => setGuest(guest < 8 ? guest + 1 : 8)} />
                </div>
            </div>
        </div>
    </div>
